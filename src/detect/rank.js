// Confidence scoring and ranking. The top-ranked candidate is what the
// one-click inline-highlight and context-menu paths use without asking the
// user to disambiguate; the toolbar popup shows the full ranked list.
...
function scoreOf(candidate) {
    if (candidate.confidence === 'exact') return CONFIDENCE_SCORE.exact
    if (candidate.hasTime && candidate.end) return CONFIDENCE_SCORE.dateTimeRange
    if (candidate.hasTime) return CONFIDENCE_SCORE.dateTime
    return CONFIDENCE_SCORE.dateOnly
}
