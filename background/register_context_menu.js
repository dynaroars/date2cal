...
// message's reference date and date-order preference come from
...
messenger.menus.onClicked.addListener(async (info) => {
    ...
    const dateOrder = context?.dateOrder ?? 'MDY'
    ...
    // fallback).
    const {candidates} = detectEvents({subject: '', body: selectionText, referenceDate, dateOrder})
